...
const chainId = Number(process.env.NEXT_PUBLIC_CHAIN_ID || CHAIN_IDS.FLARE_COSTON2)
const chain = chainId === CHAIN_IDS.BASE ? base : COSTON2_CHAIN

export const publicClient = createPublicClient({
  chain,
  transport: chain.id === CHAIN_IDS.FLARE_COSTON2
    ? createCoston2Transport()
...
export async function getInvoice(tokenId: bigint): Promise<Invoice | null> {
  try {
    const result = await publicClient.readContract({ address: addresses.invoiceNFT, abi: InvoiceNFTABI, functionName: "getInvoice", args: [tokenId] })
...
export async function getTotalInvoices(): Promise<number> {
  try {
    const result = await publicClient.readContract({ address: addresses.invoiceNFT, abi: InvoiceNFTABI, functionName: "totalInvoices" })
...
export async function getActiveInvoices(): Promise<bigint[]> {
  try {
    const result = await publicClient.readContract({ address: addresses.invoiceNFT, abi: InvoiceNFTABI, functionName: "getActiveInvoices" })
...
export async function getUserInvoiceBalance(address: Address): Promise<number> {
  try {
    const result = await publicClient.readContract({ address: addresses.invoiceNFT, abi: InvoiceNFTABI, functionName: "balanceOf", args: [address] })
    return Number(result)
  } catch { return 0 }
}
...
export async function getInvoiceOwner(tokenId: bigint): Promise<Address | null> {
  try {
    const result = await publicClient.readContract({ address: addresses.invoiceNFT, abi: InvoiceNFTABI, functionName: "ownerOf", args: [tokenId] })
...
export async function getDeposit(tokenId: bigint): Promise<Deposit | null> {
  try {
    const result = await publicClient.readContract({ address: addresses.yieldVault, abi: YieldVaultABI, functionName: "getDeposit", args: [tokenId] })
...
export async function getAccruedYield(tokenId: bigint): Promise<bigint> {
  try {
    const result = await publicClient.readContract({ address: addresses.yieldVault, abi: YieldVaultABI, functionName: "getAccruedYield", args: [tokenId] })
    return result as bigint
...
export async function getTotalValueLocked(): Promise<bigint> {
  try {
    const result = await publicClient.readContract({ address: addresses.yieldVault, abi: YieldVaultABI, functionName: "totalValueLocked" })
    return result as bigint
  } catch { return BigInt(0) }
}
...
export async function getTotalYieldGenerated(): Promise<bigint> {
  try {
    const result = await publicClient.readContract({ address: addresses.yieldVault, abi: YieldVaultABI, functionName: "totalYieldGenerated" })
    return result as bigint
  } catch { return BigInt(0) }
}
...
export async function getActiveDeposits(): Promise<bigint[]> {
  try {
    const result = await publicClient.readContract({ address: addresses.yieldVault, abi: YieldVaultABI, functionName: "getActiveDeposits" })
    return result as bigint[]
  } catch { return [] }
}
...
export async function getDecisionHistory(tokenId: bigint) {
  try {
    return await publicClient.readContract({ address: addresses.agentRouter, abi: AgentRouterABI, functionName: "getDecisionHistory", args: [tokenId] })
  } catch { return [] }
}

export async function getTotalDecisions(): Promise<number> {
  try {
    const result = await publicClient.readContract({ address: addresses.agentRouter, abi: AgentRouterABI, functionName: "totalDecisions" })
    return Number(result)
  } catch { return 0 }
}
...
export async function getAgentConfig() {
  try {
    return await publicClient.readContract({ address: addresses.agentRouter, abi: AgentRouterABI, functionName: "getConfig" })
  } catch { return null }
}
