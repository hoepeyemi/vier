# vier - Technical MVP Documentation
...
vier tokenizes invoices as Real-World Assets (RWAs) on Flare Coston2, then deploys an autonomous AI agent to:
...
- **Reduce costs** by leveraging Flare's low gas fees
...
```
Data flow:
  User -> InvoiceNFT.mintInvoice() [FDC-attested]
       -> PrivacyRegistry.commitInvoice()
       -> YieldVault.deposit()
  vier Agent polls state via RPC
       -> FtsoOracle.assessAndUpdateRisk() [live FTSOv2 price]
       -> Qwen LLM analysis
       -> AgentRouter.executeStrategy() [TEE-signed]
...
+--------------------+-----------------------------+
                     | WebSocket
+--------------------v-----------------------------+
|         VIER AGENT (TypeScript/Node.js)          |
|    FtsoOracle + AgentRouter + Qwen LLM           |
+--------------------+-----------------------------+
                     | ethers.js RPC
+--------------------v-----------------------------+
|         FLARE COSTON2 (Chain 114)                |
|  InvoiceNFT | YieldVault | PrivacyRegistry       |
|  AgentRouter | FtsoOracle -> FTSOv2              |
+--------------------------------------------------+
```
...
|-------|------------|---------|
| Network | Flare Coston2/Mainnet | Low-cost L2 execution |
...
| Contract | Address (Coston2) | Purpose |
|----------|-------------------|---------|
| InvoiceNFT | [0x7481aeE5...c9d7b7](https://coston2-explorer.flare.network/address/0x7481aeE59C35bb08c4F4B0DC4DE0C0A143c9d7b7#code) | ERC-721 invoice tokenization |
| YieldVault | [0xfDa39D8a...033b6](https://coston2-explorer.flare.network/address/0xfDa39D8a75391aAec43BB54a774bf6F03A0033b6#code) | Yield strategy management |
| PrivacyRegistry | [0xc08B41AC...791C](https://coston2-explorer.flare.network/address/0xc08B41ACeD118665d08b9517c812db483245791C#code) | Commitment + Merkle proofs |
| AgentRouter | [0x95116249...8AC5](https://coston2-explorer.flare.network/address/0x95116249980028E240403477E41dF44e12968AC5#code) | Agent decision execution |
| FtsoOracle | [0xCc351f61...94a9](https://coston2-explorer.flare.network/address/0xCc351f611b89607e25ab124D3abF58A83ADf94a9#code) | FTSOv2 price feed → on-chain risk scoring |
| MockFDCVerifier | [0x4FE685b3...aE9](https://coston2-explorer.flare.network/address/0x4FE685b35c3A1B9b297eF7D1465C49b5d859daE9#code) | FDC attestation (swap for real FDC at launch) |
...
```
Agent cycle:
  1. Poll InvoiceNFT for tokenIds
  2. FtsoOracle.assessAndUpdateRisk(tokenId)  <- live FTSOv2 price
  3. getInvoice(tokenId) + getDeposit(tokenId)
  4. Qwen LLM: analyze risk, amount, due date
  5. AgentRouter.executeStrategy(payload, sig) <- TEE-signed
...
- Flare Coston2 testnet ETH ([Faucet](https://faucet.flare.network))
- (Optional) Qwen API key for LLM features
...
QWEN_API_KEY=your_qwen_api_key  # Optional
WS_PORT=8080
...
| Flare Mainnet | 5000 | https://flare-api.flare.network/ext/C/rpc |
...
| Contract | Flare Coston2 | Flare Mainnet |
|----------|---------------|----------------|
| InvoiceNFT | TBD | TBD |
| YieldVault | TBD | TBD |
...
|-----------|--------------|------------------|
| mint() | ~250,000 | ~0.000005 C2FLR |
| deposit() | ~300,000 | ~0.000006 C2FLR |
| withdraw() | ~200,000 | ~0.000004 C2FLR |
| recordDecision() | ~150,000 | ~0.000003 C2FLR |
| changeStrategy() | ~100,000 | ~0.000002 C2FLR |

*Flare's low gas costs enable frequent agent execution*

---

**Document Version**: 1.0.0
**Last Updated**: December 2024
**Authors**: Built with AI assistance for Flare FCC Hackathon
