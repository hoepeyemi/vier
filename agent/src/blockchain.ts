...
const AGENT_ROUTER_ABI = [
  ...
  // Flare FCC: TEE-attested strategy execution path
  'function executeStrategyWithAttestation(bytes32 teeId, bytes calldata payload, bytes calldata signature) returns (uint256)',
  'function fdcVerifier() view returns (address)',
  'function teeAttestationMode() view returns (bool)',
  ...
  'event TEEAttestationVerified(bytes32 indexed teeId, uint256 indexed tokenId, uint8 strategy, uint256 confidence, uint256 nonce)',
];
...
export class BlockchainService {
  ...
  /// @notice Execute a strategy change via Flare FCC TEE attestation
  /// @dev Calls AgentRouter.executeStrategyWithAttestation() — the contract verifies the
  ///      FDC attestation signature before executing. Use this path when FCE_TEE_MODE=true.
  async executeStrategyWithAttestation(
    teeId: string,
    payload: string,
    signature: string
  ): Promise<{ success: boolean; txHash?: string; error?: string }> {
    ...
    try {
      const result = await withRetry(
        async () => {
          const tx = await this.agentRouter.executeStrategyWithAttestation(teeId, payload, signature);
          const receipt = await tx.wait();
          return { success: true, txHash: receipt.hash };
        },
        'executeStrategyWithAttestation',
        { maxAttempts: 3, baseDelayMs: 2000, maxDelayMs: 15000 }
      );
      return result;
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      console.error('Error in executeStrategyWithAttestation:', errorMessage);
      return { success: false, error: errorMessage };
    }
  }

  /// @notice Check if TEE attestation mode is active on the AgentRouter
  async isTEEAttestationModeEnabled(): Promise<boolean> {
    try {
      return await this.agentRouter.teeAttestationMode();
    } catch {
      return false;
    }
  }
